...
public class BpmAutoRuleOrgLeaderExtension extends BpmAutoRuleExtension {
	...
	public Set<String> parse(String initiatorId) {
		...
		for(UserPositionInfo userPositionInfo:userPositionInfoList){
			...
			leaderList.addAll(new HashSet(personQueryService.getOrgLeader(getCurrentUser(), userPositionInfo.getOrgId(), new Date(), new Date())));
