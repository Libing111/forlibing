...
<table width="100%">
	<tr>
		...
		<div class="singleNavMin">
			<div class="current"><span><a href="<%=path%>/tasks/category/statistics" target="frmrightChild">统计</a></span></div>
			<c:forEach var="category" items="${definitionsCategoryList}" varStatus="i">
				<!-- 
				<c:if test="${i.index == 0}">
					<div><span><a href="<%=path%>/tasks/category/home?categoryCode=${category.code}" target="frmrightChild">${category.name}</a></span></div>
				</c:if>
				<c:if test="${i.index != 0}">
	  				<div><span><a href="<%=path%>/tasks/category/home?categoryCode=${category.code}" target="frmrightChild">${category.name}</a></span></div>
				</c:if>
		...
		<div style="background-image: url('<%=contextPath%>file/img/backgrounda.png');">
			...
			-->
				<IFRAME height="100%" width="100%" frameBorder=0 id=frmrightChild name=frmrightChild src="<%=path%>/tasks/category/statistics" allowTransparency="true"></IFRAME>
			</div>
		</div>
		</td>
	</tr>
