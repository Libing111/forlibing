...
<script type="text/javascript">
	...
	function initGrid() {
		grid = $("#assignee").quiGrid({
		 ...
		 url: '<%=path%>/process/instanse/assigneeWithMe/instance/task/query?processInstanceId=${processInstanceId}&processDefinitionId=${processDefinitionId}',
		 sortName: 'id',
		 rownumbers:true,
		 checkbox:true,
         height: '100%', 
         ...
         rowAttrRender: function(rowdata, rowid){
           return null == rowdata.assigneeName ? "style=\"background-color:yellow\"" : '';
        }
		});
	}
	...
	function goback(rowindex) {
		var url = '<%=basePath %>/process/instanse/assigneeWithMe/home?categoryCode=${categoryCode}';
		window.location = url;
	}
