...
<script type="text/javascript">
	...
	function initGridGroup() {
		gridProcessDefinition = $("#processDefinition").quiGrid({
			columns:[
				{ display: 'id', name: 'id', align: 'left', hide:true},
				{ display: '名称', name: 'name', align: 'left', width: "50%"},
				{ display: '版本', name: 'version',     align: 'left', width: "50%"}
			  ],
		 url:'<%=basePath%>definitions/organization/processDefinitionQuery',
		 rownumbers:true,
		 checkbox:true,
		 enabledEdit: true,
         height: '100%',
         ...
         onSelectRow:function(rowdata, rowindex, rowDomElement){
        	gridProcessDefinitionbbb.addRow( rowdata);
            gridProcessDefinition.loadData();
         },
	...
	function initGridGroupbbb() {
		gridProcessDefinitionbbb = $("#processDefinitions").quiGrid({
			columns:[
				{ display: 'id', name: 'id', align: 'left',  hide:true},
				{ display: '名称', name: 'name', align: 'left', width: "50%"},
				{ display: '版本', name: 'version',     align: 'left', width: "50%"}
         ...
         onBeforeEdit: onBeforeEdit, 
         onSelectRow:function(rowdata, rowindex, rowDomElement){
        	 gridProcessDefinitionbbb.deleteRow( rowdata);
         },
		});
	}
	
	function mytest(){
		var data = gridProcessDefinitionbbb.getData();
		return data;
	}	
