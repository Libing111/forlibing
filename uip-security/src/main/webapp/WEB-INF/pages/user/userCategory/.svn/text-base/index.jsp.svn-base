...
<script type="text/javascript">
	...
	function onUpdate(){
		...
		Dialog.open({
			URL:'<%=basePath %>/userCategory/update?id='+row.id,
			Title:"修改",
			Width:600,
			Height:310
			});	
	...
	function onDelete(){
		...
		top.Dialog.confirm("确定要删除该记录吗？",function(){
			$.ajax({
			  type: 'POST',
			  url: '<%=basePath %>userCategory/del?ids='+row.id,
...
//停用
	function stop(){
		...
		top.Dialog.confirm("确定要停用该记录吗？",function(){
			$.ajax({
			  type: 'POST',
			  url: '<%=basePath %>userCategory/stop?id='+row.id,
			  success:function(result){
				    if(result.status == 1){
	...
	function nostop(){
		...
		top.Dialog.confirm("确定要启用该记录吗？",function(){
			$.ajax({
			  type: 'POST',
			  url: '<%=basePath %>userCategory/nostop?id='+row.id,
			  success:function(result){
				    if(result.status == 1){
						//top.Dialog.alert("发布成功！",null,null,null);
						grid.loadData();
