 ...
 <div >
	...
	<table class="tableStyle" formMode="view" align="center">	
		<tr>
			<td>名称:</td>
			<td >
				<input type="hidden" id="id" name="id" value="${raEntity.id}"/>
				<input id="name" class="validate[required]" style="width:400px" name="name" type="text" value="${raEntity.name}"/>
			</td>
		</tr>
		<tr>
			<td>编号:</td>
		...
		<tr>
			<td>注册机构分类：</td>
			<td >
				<select prompt="请选择类别" id="categoryCode" class="validate[required]" style="width:250px" name="categoryCode" data=""  selectedValue="${raEntity.categoryId}"></select>
			</td>
		</tr>
		<tr>
			<td>备注:</td>
...
<script type="text/javascript">
	...
	$('#myFormId').attr("action","<%=path%>/registrationAuthority/saveUpdate");
