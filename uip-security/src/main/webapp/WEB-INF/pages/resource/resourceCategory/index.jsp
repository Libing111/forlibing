...
<script type="text/javascript">
	...
	function initGrid() {
		grid = $("#resourceCategory").quiGrid({
		 ...
		 url: '<%=path%>/resourceCategory/query', 
		 sortName: 'id',
		 rownumbers:true,
		 checkbox:true,
         height: '100%', 
	...
	function onAdd() {
		...
		diag.Title = "新增资源分类信息";
		diag.properType="add";
	    diag.URL = "<%=basePath %>/resourceCategory/add";
	    diag.Width = 490;
	    diag.Height = 200;
...
function onUpdate(){
		...
		diag.properType="edit";
		 diag.Width = 490;
		  diag.Height = 200;
	    diag.URL = "<%=basePath %>/resourceCategory/update?id=" + id;
	    diag.OKEvent = function(){
	        var valid = diag.innerFrame.contentWindow.valids(); 
	        if(valid){     
		      	$.post(
		        	"<%=path%>/resourceCategory/updateSave",
		       		diag.innerFrame.contentWindow.getParams(),
		           	function(result){
		        		console.log(result);
		        		if(result == "SYS_SUCCESS"){
	...
	function onDelete(){
		...
		top.Dialog.confirm("确定要删除该记录吗？",function(){
			$.ajax({
			  type: 'POST',
			  url: '<%=basePath %>resourceCategory/del?ids='+row.id,
	...
	function stop(){
		...
		top.Dialog.confirm("确定要停用该记录吗？",function(){
			$.ajax({
			  type: 'POST',
			  url: '<%=basePath %>resourceCategory/stop?id='+row.id,
			  success:function(result){
				    if(result.status == 1){
	...
	function nostop(){
		...
		top.Dialog.confirm("确定要启用该记录吗？",function(){
			$.ajax({
			  type: 'POST',
			  url: '<%=basePath %>resourceCategory/nostop?id='+row.id,
			  success:function(result){
				    if(result.status == 1){
						//top.Dialog.alert("发布成功！",null,null,null);
						grid.loadData();
