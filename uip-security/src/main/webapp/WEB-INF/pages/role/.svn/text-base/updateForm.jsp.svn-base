	...
	<table class="tableStyle" id="tbaletable" style="width: 100%">	
		...
		<tr>
			<td class="properForm2Left">规则：
					<input type="hidden" name="principalIds" id="principalIds" value="${ruleKey}"/>
					<input type="hidden" name="principalValues" id="principalValues" value="${ruleValue}"/>
					<input type="hidden" name="ruleIds" id="ruleIds" value="${ruleId}"/>
			</td>
			<td >
		...
		<tr>
			...
			<td class="properForm2Right1"><input id="decription"  class="validate[length[0,200]]" style="width:400px" name="decription" type="text" value="${roleEntity.decription}"/></td>
		</tr>
	</table>
	</form>
	</div>
...
<script type="text/javascript">
	...
	function valids() {
		...
		function initComplete() {
			...
			if(ruleIds != null && ruleIds == "role.filter.rule.user"){
				$("#userNames").val(principalValues);
				$("#selectUser").show();
			}
			if(ruleIds != null && ruleIds == "role.filter.rule.department"){
				$("#deparmentNames").val(principalValues);
				$("#selectDepartment").show();
			}
			if(ruleIds != null && ruleIds == "role.filter.rule.post"){
				$("#postNames").val(principalValues);
				$("#selectPost").show();
			}
			//表单提交
			$('#myFormId').submit(
