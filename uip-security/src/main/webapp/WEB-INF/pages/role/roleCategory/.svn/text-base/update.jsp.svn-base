...
<body>
 <div>
	<form id="myFormIda"  method="post" action="<%=path%>/roleCategory/updateSave" target="frmright" failAlert="表单填写不正确，请按要求填写！">
	<table class="tableStyle" formMode="view" align="center">	
	
		<tr style="height: 60px">
			<td style="width: 60px">名称:</td>
			<td >
				<input type="hidden" id="id" name="id" value="${roleCategoryEntity.id}"/>
				<input id="name" class="validate[required,length[0,20]]" style="width:400px" name="name" type="text" value="${roleCategoryEntity.name}"/>
			</td>
		</tr>
		<tr style="height: 60px">
			<td style="width: 60px">编号:</td>
			<td><input id="code" style="width:400px" class="validate[required,length[0,20]]"  name="code" type="text" value="${roleCategoryEntity.code}"/></td>
		</tr>
		<tr style="height: 60px">
			<td style="width: 60px">序号:</td>
			<td><input id="sequenceNumber" style="width:400px" class="validate[required,custom[onlyNumber],length[0,20]]" name="sequenceNumber" type="text" value="${roleCategoryEntity.sequenceNumber}"/></td>
		</tr>
		<tr style="height: 60px">
			<td style="width: 60px">备注:</td>
			<td><input id="description" style="width:400px" name="description" class="validate[required,length[0,200]]" type="text" value="${roleCategoryEntity.description}"/></td>
		</tr>
		<tr style="height: 60px">
		    <td colspan="2">
