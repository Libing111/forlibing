 ...
 <div >
	...
	<table class="tableStyle" id="tbaletable" style="width: 100%">	
		...
		<tr>
			...
			<td class="properForm2Right1"><div prompt="请选择资源" id="resourceId"  class="suggestion"  data=""  showList="true" boxWidth="200" inputWidth="200" selectedValue="${applicationAndGroup.resourceId}" ></div></td>
		</tr>
		<tr>
...
<script type="text/javascript">
	...
	var resourceList = <%=JsonUtil.array2JSON(request.getAttribute("resourceList"))%>;
	var resourceData = {"list":[]};
	var resource;
	for(var i = 0; i < resourceList.length; i++){
		resource = {'value':resourceList[i].id, 'key':resourceList[i].name,'suggest':resourceList[i].name};
		resourceData.list.push(resource);
	}
	$('#resourceId').data("data",resourceData);
	$('#resourceId').render();
	...
	function getResource(){
	   var resourceId = $("#resourceId").attr("relValue");
		return resourceId;
	}	
